...
def load_database_federated_non_iid(root_path, csv_path, batch_size, num_clients, image_size=(128,128), test_size=None, as_rgb=False, is_stream=False):
    """load images from csv and split into train and test resulting partitions of clients on Federated learning
    ...
    tf_image = {
        ...
        "noise": transforms.Compose([
                transforms.Resize(image_size),
                transforms.ToTensor(),
                transforms.v2.GaussianNoise(mean=0, sigma=0.1),
                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ]),
        "illumination": transforms.Compose([
                transforms.Resize(image_size),
                transforms.ToTensor(),
                CustomTransformIllumination(brightness=10, contrast=1.4),
                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ])
    }
...
class CustomDatasetFromCSVNonIID(Dataset):
    """Generating custom dataset for importing images from csv
    """    
    def __init__(self, path_root, tf_image, csv_name, domains, as_rgb=False, task=None):
        table_data = pd.read_csv(csv_name)
        if task is not None:
            table_data.query("Task == @task", inplace=True)
        ...
        self.targets = table_data["y"].tolist()
        self.domais = domains
        self.domains_n = len(self.domains)
...
def load_database_federated_non_iid(root_path, csv_path, batch_size, num_clients, image_size=(128,128), test_size=None, as_rgb=False, is_stream=False):
    ...
    tf_image = {
        ...
        "occlusion": transforms.Compose([
                transforms.Resize(image_size),
                transforms.ToTensor(),
                CustomTransformOcclusion(occlusion_size=8),
                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ])
    }
...
class CustomTransformOcclusion:
    ...
    def __call__(self, x):
        ...
        h, w, _ = image.shape
        noise_image = cv2.rectangle(noise_image, (w, h), (w - (w//self.occlusion_size), h - (h//self.occlusion_size)), (255, 255, 255), cv2.FILLED)
        noise_image = np.clip(noise_image, 0, 255).astype(np.uint8)
